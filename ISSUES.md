...
### Switch Brave proxy from `/web/search` to the LLM Context endpoint
[netlify/edge-functions/web-search.ts](netlify/edge-functions/web-search.ts) currently calls `https://api.search.brave.com/res/v1/web/search`, which returns ranked links with short `description` snippets (~1–2 sentences) meant for human-readable result lists. The orchestrator in [src/services/webSearch/webSearchOrchestrator.ts](src/services/webSearch/webSearchOrchestrator.ts) then grounds the LLM on just those snippets via `searchResultsToDocuments` in [webSearchClient.ts:90](src/services/webSearch/webSearchClient.ts:90) — so model answers are constrained to whatever fits in a Brave preview. Brave ships an "LLM Context" / summarizer endpoint explicitly tuned for agent consumption (richer extracts, machine-readable shape) that would close the biggest remaining gap in our web-RAG pipeline without us having to fetch + chunk + rerank pages ourselves. Work: confirm the endpoint path and auth from Brave's docs, update `queryBrave` to hit it, re-map the response into `NormalizedResult` (or widen the type if extracts are longer), and verify the existing `documents` injection still flows. The system prompt at [webSearchOrchestrator.ts:42](src/services/webSearch/webSearchOrchestrator.ts:42) probably needs no change — citations still key off document index. Plan key + quota check first; the LLM endpoint may be on a different tier than the standard web search subscription.

### Migrate HF edge functions to `@huggingface/inference` SDK
[netlify/edge-functions/chat.ts](netlify/edge-functions/chat.ts) and [netlify/edge-functions/embeddings.ts](netlify/edge-functions/embeddings.ts) hand-roll `fetch` + URL construction + auth headers against the HF router. The `@huggingface/inference@4.13.12` SDK is already a dependency but unused. Static inspection confirms it's Deno-compatible (pure ESM, standard `fetch` only, no `process.*` / `node:*` / `Buffer` in the hot path); both functions can be migrated together via `npm:@huggingface/inference@4` imports. Wins: HF absorbs router-URL changes for us (the hardcoded `router.huggingface.co/hf-inference/...` path is HF's older pattern), `chatCompletionStream` and `featureExtraction` replace ~30 lines of boilerplate each, the SDK normalizes the 2D/3D embedding response shape so the local `normalizeEmbeddings` helper in [embeddings.ts:52](netlify/edge-functions/embeddings.ts:52) can go. **All-or-nothing**: both HF edge functions migrate in one PR (mixed fetch + SDK is worse than either pure choice). `web-search.ts` is unaffected — it talks to Brave, not HF.
